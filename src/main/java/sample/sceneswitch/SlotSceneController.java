...
public class SlotSceneController implements Initializable {
        ...
        else
        {
            ...
            for (Room room : rooms)
            {
                if (room.getRoom_name().equals(rname))
                {
                    ...
                    alert.setTitle("Slots");
                    alert.setHeaderText("Slot Adding");
    ...
    public void delete_slot(MouseEvent e)
    { ////////////// WE ARE STILL HERE
        ...
        else
        {
            ...
            for (Room room : rooms)
            {
                if (room.getRoom_name().equals(roomchoice1.getSelectionModel().getSelectedItem()))
                {
                    alert.setAlertType(Alert.AlertType.CONFIRMATION);
                    alert.setTitle("Slots");
                    alert.setHeaderText("Slot Deleted");
                    alert.setContentText("Are you sure you want to delete this slot");
                    if (alert.showAndWait().get() == ButtonType.OK)
                    {
                        if( room.List_of_Slots.removeIf(slot -> slot.getDate().equals(SlotDate1.getValue()) && slot.getTimef().equals(timef) && !slot.getReserved()))
                        {
                        ...
                        else
                        {
                            ...
                            alert.setTitle("Slots");
                            alert.setHeaderText("Delete slots");
                            alert.setContentText("This slot is reserved \n you can't delete it");
                            alert.setX(HelloApplication.stg.getX() + 200);
                            alert.setY(HelloApplication.stg.getY() + 215);
                            alert.show();
                            break;
